...
def _plot_heatmaps_for_subset(df, output_dir, file_suffix="", self_map=False, node_name=None):
    ...
    for freq in frequencies:
        ...
        prefixes = [node_name] if self_map and node_name else ["CC", "LW"]
        for prefix in prefixes:
            ...
            for (i, j), val in np.ndenumerate(adj_matrix_test_power.values):
                ...
                baseline_raw = adj_matrix_baseline_power.values[i, j]
                outlier_raw = adj_matrix_outliers.values[i, j]
                baseline_val = baseline_raw
                outlier_percentage = outlier_raw
                ...
                if baseline_val != "null":  # Only plot for valid values
                    # Plot the bottom half rectangle for the baseline_mean value
                    plt.gca().add_patch(plt.Rectangle(
                        (x + 0.02, y + 0.52), 0.96, 0.46,  # x, y, width, height
                ...
                if outlier_percentage != "null":  # Only plot for valid values
            ...
            if self_map and node_name:
                plt.suptitle(
                    f"Pairwise Power Measurement (Between B205 & B210) Heatmap - Frequency {freq}, Node {node_name} \n \
                    Top Half - Test Power Mean; Bottom Half - Baseline Power Mean; Circle - Outlier Percentage"
                    , fontsize=14, ha="center", multialignment="center")
            else:
                plt.suptitle(
                    f"Pairwise Power Measurement Heatmap - Frequency {freq}, Prefix {prefix}\n \
                    Top Half - Test Power Mean; Bottom Half - Baseline Power Mean; Circle - Outlier Percentage"
                    , fontsize=14, ha="center", multialignment="center")
            ...
            filename = f"power_heatmap_freq_{freq}_{prefix}{suffix}.png"
...
def plot_heatmap_with_outliers(input_file, output_dir):
    ...
    df["receiver"] = df["receiver"].fillna("None")
    df["transmitter_channel"] = df["transmitter_channel"].fillna("None")
    df["receiver_channel"] = df["receiver_channel"].fillna("None")

    df_main_tests = df[df["transmitter"] != df["receiver"]].copy()
    df_self_tests = df[
        (df["receiver_frequency"] == "3.32G")
        & ((df["transmitter"] == df["receiver"]) | (df["transmitter"] == "None"))
    ].copy()

    if not df_main_tests.empty:
        _plot_heatmaps_for_subset(df_main_tests, output_dir, file_suffix="")

    if not df_self_tests.empty:
        # Self-tests use diagonal block layout: one image per node prefix
        # Pre-processing: combine transmitter/receiver and serial_number columns to create unique identifiers
        # Convert serial number to radio type to separate radios on the same node
        tx_radio = df_self_tests["transmitter_serial_number"].fillna("None").apply(get_radio_type)
        rx_radio = df_self_tests["receiver_serial_number"].fillna("None").apply(get_radio_type)
        # Make new column to store raw node ID
        df_self_tests["node_id"] = df_self_tests["receiver"]  # or receiver, they are the same for self-tests
        df_self_tests["transmitter"] = df_self_tests["transmitter"] + "[" + tx_radio + "]"
        df_self_tests["receiver"] = df_self_tests["receiver"] + "[" + rx_radio + "]"

        # Fill in blank baseline with default value
        df_self_tests["baseline_power_mean"] = df_self_tests["baseline_power_mean"].fillna("null")
        for node_prefix in df_self_tests["node_id"].unique():
            df_subset = df_self_tests[df_self_tests["node_id"] == node_prefix]
            _plot_heatmaps_for_subset(df_subset, output_dir, file_suffix=f"self", self_map=True, node_name=node_prefix)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-dir", default=".", help="Directory to save the plots to", required=False)
